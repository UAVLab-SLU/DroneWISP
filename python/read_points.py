...
def plot_3d_interactive(path):
    ...
    fig.add_scatter3d(x=[point[0] for point in points], y=[point[1] for point in points],
                      z=[point[2] for point in points],mode = 'markers',marker=dict(size=1,color='red'))

    # fig.update_traces(visible='legendonly')
    import os
    if os.path.exists('points.html'):
        os.remove('points.html')
    fig.write_html('points.html')
    plt.close()


if __name__ == '__main__':
    plot_3d_interactive('../run/motorBike/constant/polyMesh/points')
